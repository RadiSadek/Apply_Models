...
source(paste(base_dir,"/Apply_Models/Logistic_CityCash_Prescore.r", sep=""))
...
scoring_df <- gen_apply_score(
  ...
  flag_judicial,0,flag_third_side,flag_cashpoint,base_dir,1)
...
final_exists <- read.xlsx(paste(base_dir,
  "\\Monitoring\\Files\\Scored_Credits_Prescore.xlsx", sep=""))
final <- rbind(final_exists, final)
write.xlsx(final, paste(base_dir,"\\Monitoring\\Files\\Scored_Credits_Prescore.xlsx", 
  sep=""))
