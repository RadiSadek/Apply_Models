...
SELECT id, status, date, product_id
FROM ",db_name,".credits_applications 
WHERE status in (4)",sep="")))
select_credits <- fetch(get_actives_sql,n=-1)


# Apply select criteria
select_credits <- subset(select_credits,select_credits$date>="2020-03-01" & 
                         select_credits$date<="2020-04-30")


# Remove credits with already an offer 
po_sql_query <- paste(
  "SELECT application_id, created_at FROM ",db_name,
  ".prior_approval_refinances",sep="")
...
daily$left_to_pay <- daily$final_credit_amount - daily$paid_hitherto
daily$paid_ratio <- daily$paid_hitherto / daily$final_credit_amount


# Filter those whose passed installments are lower than 30%
...
# Subset on criteria based on paid ratio (>30%) and max DPD of 180 days
...
write.xlsx(daily,"C:\\Users\\rsadek.CC\\Desktop\\offers_refinance.xlsx")
...
# Output results
write.xlsx(daily,
  "C:\\Projects\\Prior_Approval\\Prior_Approval_v2\\Refinance\\results\\offers_refinance.xlsx")
