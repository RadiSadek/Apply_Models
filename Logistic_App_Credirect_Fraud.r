...
gen_app_credirect_fraud <- function(df,scoring_df,products,
  ...
  check_phone <- gen_query(con,gen_get_phone_numbers(db_name,all_df))
  check_phone <- as.data.frame(check_phone[!duplicated(check_phone$number),])
  if(nrow(check_phone)>1){
      fraud_flag <- 1
  }
  
  # Apply email criteria
  check_email <- gen_query(con,gen_get_email(db_name,all_df))
  check_email <- as.data.frame(check_email[!duplicated(check_email$email),])
